...
function App() {
  ...
  const sendDistances = async () => {
    ...
    try {
      await fetch("http://localhost:8000/floor_updates", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          technicians: technicians.map((tech) => ({
            id: tech.id.toString(),
            location: { x: tech.column, y: tech.row, z: tech.floor },
          })),
          rack_ids: racks.map((rack) => rack.id.toString()),
          rack_locations: racks.map((rack) => ({
            x: rack.column,
            y: rack.row,
            z: rack.floor,
          })),
